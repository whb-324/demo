...
//		}catch(Exception e){}
	...
	@Override
	public List<CustomerInfo> getCustomerListById(int id) {
		//查询账户下的所有客户
		return customerInfoDao.getAllCustomer(id);
	}

	@Override
	public List<CustomerInfo> getCustomerList(String likes) {
		//模糊查询客户
		List finalList = customerInfoDao.findByName(likes);
		finalList.remove(customerInfoDao.findByRegionCode(likes));
		finalList.addAll(customerInfoDao.findByRegionCode(likes));
		finalList.remove(customerInfoDao.findByTelCode1(likes));
		finalList.addAll(customerInfoDao.findByTelCode1(likes));
		return finalList;
	}
	...
	public Response updateCustomerInfo(CustomerInfo obj) {
		//将新的对象判断后放入数据库
		CustomerInfo customer =(CustomerInfo) customerInfoDao.findById(obj.getID());
